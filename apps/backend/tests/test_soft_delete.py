...
@pytest.mark.asyncio
async def test_soft_delete_flow(db_session):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        f_list = await client.get(f"/api/v1/feedings/baby/{baby.id}")
        assert len(f_list.json()) == 1

        s_list = await client.get(f"/api/v1/sleep/baby/{baby.id}")
        assert len(s_list.json()) == 1

        # 5. Soft-delete feeding
        del_f = await client.delete(f"/api/v1/feedings/{feeding_id}")
        ...
        f_list2 = await client.get(f"/api/v1/feedings/baby/{baby.id}")
        assert len(f_list2.json()) == 0

        s_list2 = await client.get(f"/api/v1/sleep/baby/{baby.id}")
        assert len(s_list2.json()) == 1

        # 7. Check deleted activities list (should show feeding)
        deleted_list = await client.get(f"/api/v1/activities/deleted/baby/{baby.id}")
        ...
        # 8. Restore feeding
        restore_res = await client.post(f"/api/v1/activities/restore/feed/{feeding_id}")
        assert restore_res.status_code == 200
        assert restore_res.json()["status"] == "success"

        # 9. Verify feeding is active again and no longer in deleted activities list
        f_list3 = await client.get(f"/api/v1/feedings/baby/{baby.id}")
        assert len(f_list3.json()) == 1

        deleted_list2 = await client.get(f"/api/v1/activities/deleted/baby/{baby.id}")
        assert len(deleted_list2.json()["feedings"]) == 0

        # 10. Test invalid kind / non-existent item edge cases
        err_res = await client.delete(f"/api/v1/feedings/99999")
        assert err_res.status_code == 404

        err_res2 = await client.post(f"/api/v1/activities/restore/invalid_kind/{feeding_id}")
        assert err_res2.status_code == 400

        err_res3 = await client.post(f"/api/v1/activities/restore/feed/99999")
